...
function sieve(N :: Int)
  ...
  for i = 1:N
    if(is_primes[i] == true) # is i is a prime, we can prepare to fill all multiples with true
      # j = i*i - that's the original; but I tihnk, we should start filling the sieve with true from position 2*i onwards...
      j = 2*i # like this
      if j > N
        return find(is_primes) # if the first true would exceed the array, we are done and return the indices of the true's
      else
