...
manhattan_dist(p::Point) = abs(p.x) + abs(p.y)

function move_one(p :: Point, inst)
    # function takes a starting point and an "instruction" of the form AN*
    # it starts at the starting point, moves along the instruction, and returns all points it touches along the instruction

    cmd  = inst[1] # get the command
    num = parse(Int, inst[2:end])
...
function move_cmds(cmds)
    # takes a comma-separated list of commands and returns a list of points the path described by the commands visits
    strt = Point(0,0)
    res = Point[]
    push!(res, strt)
    cmds_split = split(cmds, ",")
    for el in cmds_split # for every command,
        segment = move_one(strt, el) # obtain the points visited (starting from the last point of the previous segment)
        res = vcat(res, segment)
        strt = segment[end]
    end
    res
end
...
function find_intersections(s1, s2)
    # find intersections between sequence of points 1 & 2; this has quadratic complexity in the size of the seq's
    res = Point[]
    for e1 in s1 # run through first seq
        for e2 in s2 # and second seq
            if e1 == e2 # compare if points are the same
                push!(res, e1) # and add to result vector
            end
        end
    end
    res
...
for el in itscts # for every intersection
    len1 = findfirst(x -> x == el, res1) - 1 # find the length of path1
    len2 = findfirst(x -> x == el, res2) - 1# and length of path2
    @show el, len1+len2
    push!(paths, len1+len2)
end
