# IDEA:
# We run through the tree depth-first and for every node, we update a map node -> depth

# tree data structure based on https://stackoverflow.com/questions/36593490/tree-data-structure-in-julia
...
function buildTree(s)
    # take the string s and build up a tree
    tree = Dict{String, Vector{String}}() # the tree structure containing parent -> [children]
    ...
    for l in s
        ...
        push!(nodes, parent)
        push!(nodes, child)
        if(haskey(tree, parent)) # there is already an entry in the dict -> update list of children
           push!(tree[parent], child)
        else # no children yet -> create a list with one entry
           tree[parent] = [child]
        end
    end
    for n in nodes # run across all nodes
        if(!haskey(tree, n)) # if they are not in the tree (as leaves)
           tree[n] = [""] # add them to the tree
        end
    end
    tree
end
